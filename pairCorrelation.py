# compute g(r) for a LAMMPS data file, output a numpy file, report intrinsic cutoff 
...
# normalization (placeholder — user noted this is incomplete)
for p in pairs:
	pair_gr[p] *= 2
...
for i in range(len(r)):
    vals = " ".join(f"{pair_gr[p][i]:.6f}" for p in pairs)
    print(f"{r[i]:.6f} {vals}")

print(f"\nPair first-peak cutoffs: { {p: f'{cutoffs[p]:.4f}' for p in pairs} }")
print(f"Intrinsic neighborhood cutoff: {intrinsic_cutoff:.4f}")
